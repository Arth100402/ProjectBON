    ...
    <form method="POST" action="{{ route('bon.update', $bon->id) }}" enctype="multipart/form-data">
        ...
        <div class="form-group">
            ...
            <input type="number" min="0" value="{{ $bon->total }}" name="biayaPerjalanan"
                class="form-control" id="biayaPerjalanan" readonly>
            @error('debit')
                <span class="text-danger">{{ $message }}</span>
            @enderror
        </div>
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    $.each(data, function(i, item) {
                        var row = '<tr data-toggle="collapse" style="' + (item.deleted_at ?
                                "text-decoration:line-through" : "") + '" href="#collapse' +
                            item.id +
